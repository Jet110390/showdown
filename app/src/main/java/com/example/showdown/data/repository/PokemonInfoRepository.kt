...
//    val pokeList: MutableList<PokeInfoDto> = mutableListOf()
    suspend fun getPokeUrls(): List<Urls>? {
        return try {
         ...
         try{
           for(i in 0..pokeInfo.size-1){
               ...
               if (response1.isSuccessful){
                   val speciesNumber = pokeInfo[i].species.url.replace("https://pokeapi.co/api/v2/pokemon-species/","").takeWhile { it.isDigit() }.toInt()
                   var flavTextData = response1.body()?.flavorTextEntries?.find { it.language.name.equals("en")}
                   val genus= response1.body()?.genera?.find { it.language.name.equals("en") }
//                   val genus = response1.body()?.genera?.get(7)?.genus
                   val evolvesFrom = response1.body()?.evolvesFromSpecies?.name ?: "no pre-evolution"
...
//                               && it.pokemon.name != varieties[0].pokemon.name+"-gmax"
                               ...
                               && it.pokemon.name != varieties[0].pokemon.name.replace("-50", "-10")
...
//                 )
                 ...
                 Log.d("getSpecies exception","cause: ${ex.cause} ${ex.message} ${ex.localizedMessage}")
                 null
             }
         return speciesList
     }
...
//    }
     ...
     suspend fun getPokedex( pokemonInfo: MutableList<PokeInfoDto>,speciesInfo: MutableList<SpeciesEntry>): DataState<List<Pokemon>>  {
...
//                     Log.d("i=$i"," spec= ${speciesInfo[i]} info=${pokemonInfo[i].name}")
                     pokeList.add(
                     DexEntry(pokemonInfo[i].id,
                         pokemonInfo[i].name,
...
//                                                   legendsImgs[i]
                         ...
                         if(pokemonInfo[i].types.size >1){pokemonInfo[i].types[1]?.type?.name}else{"none"},
